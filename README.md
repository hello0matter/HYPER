...
服务器模式默认开启“统一策略模拟”。模拟策略始终运行；真实下单总开关和真实策略开关只决定是否把本轮刚出现的同一个模拟开仓事件额外发送为真实订单。真实关闭时继续积累模拟记录，真实开启时两边共用同一份 WS Z、方向、入场过滤、退出规则和最多持仓限制。真实订单若因余额、最低金额、杠杆或成交失败而没有执行，会作为执行差异保留，不会让真实模块自行改选另一个信号。网页仍可切回“独立模拟（旧模式）”。
...
统一策略模拟收益仍是残差/Z 回归近似。真实开关开启后，真实模块只消费同一轮统一策略产生的新开仓事件；真实盘仍以逐笔成交价、盘口深度、滑点、手续费、资金费和官方仓位为准。
...
默认真实执行风格为“极速锁定”：账户余额和仓位由独立后台线程每 3 秒刷新，商机出现后不再同步请求账户接口；流程默认是“读取缓存账户 → 准备/复用杠杆设置 → 内存 l2Book 最终复查 → 发送真实 IOC → 写入模拟记录 → 后台刷新账户”。Dashboard 可以选择极速、平衡、严格风格，也可以上下调整步骤、删除可选步骤或插入“最新账户查询”等步骤。真实 IOC 与模拟记录是必需步骤，不能删除。
