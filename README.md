...
默认真实执行风格为“极速锁定”：账户余额和仓位由独立后台线程每 3 秒刷新，商机出现后不再同步请求账户接口；流程默认是“读取缓存账户 → 准备/复用杠杆设置 → 内存 l2Book 最终复查 → 发送真实 IOC → 写入模拟记录 → 后台刷新账户”。Dashboard 日常只需要选择极速、平衡、严格三种风格。底层步骤编辑器放在默认折叠的“专家调试模式”中，主要用于定位漏单与测量步骤耗时；一般交易不需要打开。真实 IOC 与模拟记录是必需步骤，不能删除，下单前检查也不能移动到 IOC 之后。
...
测试覆盖“旧扫描标签不得阻止当前 WS 信号”“Z 变号后必须重算双腿方向”“网页诊断与真实交易循环共用同一套过滤规则”“WS 信号不能重复开仓”以及“实时 Z 回归后自动平仓”。
