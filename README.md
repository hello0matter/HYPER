...
Dashboard 的“多市场组合”不是把一个 BTC 策略硬套到所有产品，而是按产品分别选择策略：默认覆盖 SPY、QQQ、债券、黄金、白银、能源、外汇和 BTC/ETH 等 20 个代码。每个产品只用前约70%的历史在 62 个固定技术规则中选一个，冻结参数后用最后约30%做样本外考试，合格产品再按等权合成组合；不合格产品默认留现金。手续费、历史年数、是否允许做空、最低交易数、选择段回撤门槛和产品列表都可以在弹窗修改。
...
组合修正结果和风险档位对比记录在 [`docs/multi_asset_portfolio_results.md`](docs/multi_asset_portfolio_results.md)。4年均衡档样本外约 +4.85%，但同期等权买入持有约 +23.44%；2倍激进档约 -32.79%。这说明当前策略优势不足，不能用加杠杆掩盖。
