...
Dashboard 顶部“策略实验室”目前覆盖 7 类、31 组固定参数：EMA 趋势、MACD、RSI 均值回归、布林均值回归、唐奇安突破、Supertrend 和动量趋势。它使用前 60% K线作为训练段、后 40% 作为样本外段；信号在本根收盘计算，按下一根开盘成交，并扣除配置的估算往返成本。排行榜优先显示训练和样本外同时为正、样本外交易数与盈亏比达到最低研究门槛的组合。
...
Hyperliquid 公开 `candleSnapshot` 目前约有 5000 根 K 线的历史上限，因此 5m 实际最多约 17 天、15m 约 52 天、1h 约 208 天。页面会显示每个币真正取得的 K 线数量和覆盖天数，不会把用户填写的 90 天当成已经完整获取。需要更长的 5m/15m 回测时，应接入 TradingView 导出或有授权的历史数据源，再用 Hyperliquid 最近数据做执行市场复验。
