...
Dashboard 的“新币启动雷达”通过 GeckoTerminal 公开接口每5分钟扫描 TON 最新池子，综合池子年龄、流动性、1小时/24小时成交额、成交额相对流动性的换手率、买卖笔数和价格动量。默认只有年轻、流动性和成交强度达标、买盘占优、价格刚转强且1小时没有过度拉升的池子才成为“启动候选”；其余显示预热或具体过滤原因。候选只会进入独立 SQLite 模拟盘，按可配置止盈、止损、移动止盈、最长持有时间和流动性坍塌规则退出，默认估算往返成本300bps。

Kraken 自选观察默认包含 EVAAUSD，使用最近721根15分钟K线计算1小时/6小时/24小时动量、成交量倍数、EMA20/60和24小时突破。EVAA 不在 Hyperliquid 永续市场，因此这个面板不会使用 Hyperliquid API 钱包交易 EVAA。新池公开行情无法识别合约后门、持币集中、撤池、同名假币或只能买不能卖，当前功能严格保持观察和模拟用途。

### 热点情绪研究

Dashboard 的“热点情绪研究”默认自动读取 Google Trends US/HK 今日热点，也允许手动填写 `热点词=资产代码` 映射。历史回测使用 GDELT 全球新闻日线衡量某个词相对过去的异常程度，并与 Yahoo Finance 股票、ETF或加密资产日线对齐。信号同时要求新闻Z和热度倍数达标、收盘价位于 EMA20 上方、成交量放大且近5日尚未过热；当天收盘确认后只按下一交易日开盘分批进入，避免使用尚未产生的信息。

退出规则包括热点降温、固定止损、固定止盈、移动止盈和最长持有，结果扣除可配置往返成本并保存到 SQLite。Google Trends 没有在这里提供稳定完整的官方历史搜索 API，因此“今日发现”和“历史回测”使用不同来源；该功能目前严格用于研究，不连接模拟盘或真实交易。
